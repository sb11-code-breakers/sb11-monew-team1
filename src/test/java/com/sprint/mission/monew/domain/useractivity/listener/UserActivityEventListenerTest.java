  ...
  class SubscriptionCreated {
    ...
    void handle_SubscriptionCreated() {
      SubscriptionCreatedEvent event = new SubscriptionCreatedEvent(
          UUID.randomUUID(), UUID.randomUUID(), "IT", Instant.now()
      );
  ...
  class CommentLiked {
    ...
    void handle_CommentLiked() {
      CommentLikedEvent event = new CommentLikedEvent(
          UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), "기사 제목", Instant.now(), Instant.now()
      );
      listener.handle(event);
  ...
  class ArticleViewed {
    ...
    void handle_ArticleViewed() {
      ArticleViewedEvent event = new ArticleViewedEvent(
          UUID.randomUUID(), UUID.randomUUID(), "NAVER", "URL", "제목", Instant.now(), "요약", Instant.now()
      );
      listener.handle(event);
