...
-- 3. 관심사 15개로 확장 (유저당 10개씩 구독해야 하므로 풀을 늘림)
...
-- 6. 구독 정보 매핑 (★유저당 정확히 10개씩 구독, 총 1,500건)
...
-- 7. 댓글 데이터 적재 (★유저당 정확히 50개씩 작성, 총 7,500건)
...
-- 8. 댓글 좋아요 이력 적재 (★유저당 정확히 20개씩 분산, 총 3,000건)
...
FROM (
         ...
         FROM (SELECT id, ROW_NUMBER() OVER (ORDER BY id) as id_num FROM users) u
                  CROSS JOIN (SELECT id, ROW_NUMBER() OVER (ORDER BY id) as id_num FROM comments) c
     ) sub
WHERE rn <= 20;

-- 9. 기사 조회 이력 적재 (★유저당 정확히 10개씩 분산, 총 1,500건)
...
FROM (
         ...
         FROM (SELECT id, ROW_NUMBER() OVER (ORDER BY id) as id_num FROM users) u
                  CROSS JOIN (SELECT id, ROW_NUMBER() OVER (ORDER BY id) as id_num FROM articles) a
     ) sub
WHERE rn <= 10;