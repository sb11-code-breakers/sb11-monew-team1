...
import { BASE, headers, pick, userIds, articleIds, comments } from '../speed/common.js';
...
function fireBurst(url, uid, name) {
  const params = { headers: headers(uid), tags: { name } };
...
// ── setup: C3는 관심사 id가 필요(medium 시드엔 없음) → 조회해 확보, 없으면 명확히 중단 ──
export function setup() {
  if (CASE !== 'c3') return {};
  const uid = pick(userIds);
  const res = http.get(`${BASE}/api/interests?orderBy=name&direction=ASC&limit=100`, { headers: headers(uid) });
  const body = res.json();
...
export default function (data) {
  switch (CASE) {
    case 'c2': { // 댓글 좋아요 — 같은 (user, comment) 동시 중복
      ...
      analyze(fireBurst(`${BASE}/api/comments/${c.id}/comment-likes`, uid,
    ...
    case 'c3': { // 관심사 구독 — 같은 (user, interest) 동시 중복
      ...
      analyze(fireBurst(`${BASE}/api/interests/${interestId}/subscriptions`, uid,
    ...
    case 'c4': { // 기사 조회수 — 같은 (user, article) 동시 (500 재현 — 핵심)
      ...
      analyze(fireBurst(`${BASE}/api/articles/${articleId}/article-views`, uid,
